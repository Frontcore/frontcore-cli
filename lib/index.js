	...
	module.exports = function(options) {
		...
		if (process.argv.length < 3) {
			...
			switch (process.argv[2].toLowerCase()) {
				...
				case 'init':
					if (process.argv[3] && (!process.argv[4] || process.argv[4])) {
						if (process.argv[3].toLowerCase() === '--path' || process.argv[3].toLowerCase() === '-p') {
							if (process.argv[4]) {
								if (check.isPathExist(path.normalize(process.argv[4]))) {
									if (check.isDir(path.normalize(process.argv[4]))) {
										options.projPath = path.normalize(process.argv[4]);
									} else {
								...
								msg.error('No project directory path specified with ' + process.argv[3].toLowerCase());
								isExit = true;
							}
						} else {
				...
				case 'load':
					if (process.argv[3] && (!process.argv[4] || process.argv[4])) {
						if (process.argv[3].toLowerCase() === '--load' || process.argv[3].toLowerCase() === '-l') {
							if (process.argv[4]) {
								if (check.isPathExist(path.normalize(process.argv[4]))) {
									if (check.isFile(path.normalize(process.argv[4]))) {
										send.config(path.normalize(process.argv[4]));
									} else {
						...
						if (check.isPathExist(__dirname + '/frontcore.json')) {
							send.config(__dirname + '/frontcore.json');

							msg.info([
								'\n ' + PRODUCT.name + ' v' + PRODUCT.version + ' is connected to frontcore server',
								'',
